@php
    ...
    $payrollLink = ['label' => 'Payroll', 'route' => 'hr.payroll.index', 'icon' => 'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z'];
    ...
    $hrLinks = [
        ...
        ['label' => 'Attendance & Leave', 'route' => 'hr.attendance.index',  'icon' => 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4'],
        ...
        ['label' => 'Policy',     'route' => 'hr.policy.index',      'icon' => 'M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z'],
    ];
    ...
    $employeeLinks = [
        ...
        ['label' => 'Attendance & Leave', 'route' => 'employee.attendance.index',   'icon' => 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4'],
        ...
        ['label' => 'Policy',     'route' => 'employee.policy.index',      'icon' => 'M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z'],
    ];

    // Payroll module is limited to Head Office HR and payroll workflow roles.
    if ($user?->is_hr && app(\App\Services\Hr\HrScopeService::class)->canAccessPayrollModule($user)) {
        array_splice($hrLinks, 3, 0, [$payrollLink]);
    }

    $isPayrollApprover = $user && ! $user->is_hr && $user->canAccessPayrollWorkflow();

    $links = $user?->is_hr ? $hrLinks : ($isPayrollApprover ? [$payrollLink] : $employeeLinks);

    if ($user?->hasRole('deputy_md')) {
        $links[] = [
            'label' => 'Manage Signatures',
            'route' => 'hr.signatures.index',
            'icon'  => 'M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z',
        ];
    }
    $roleLabel = $user?->is_hr
        ? 'HR'
        : ($isPayrollApprover ? \App\Models\Role::labelFor($user->primaryPayrollRole()) : 'Employee');
@endphp
